...
5. Apply the bundled Wiley-style publication preset: Arial, white background, near-square page, four-sided black frame, inward ticks on the bottom and left axes only, no ticks on the top or right frame, no grid, and a compact frameless legend. Use the lower-left corner for EQE so the legend does not cover the long-wavelength curves; keep JV in the lower-right corner. Each legend row uses a colored line segment with the same color and visual width as its curve, followed by the curve name. JV and EQE are both line-only; do not add symbols to EQE curves.
